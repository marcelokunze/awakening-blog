...
const isValidCategory = (category: string): category is CategoryType => {
  return ['Science', 'Guide & Tips', 'Techniques', 'Origins', 'Journey', 'Product'].includes(category)
}
...
export async function getPostBySlug(slug: string): Promise<PostMeta | null> {
  ...
  if (!matterData.category || !isValidCategory(matterData.category)) {
    throw new Error(`Invalid or missing category in post: ${slug}`)
  }
