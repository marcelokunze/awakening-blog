import React from "react"
import { Card, CardContent } from "@/components/ui/card"
import BreathingOrb from "./BreathingOrb"
import StartNowButton from "./StartNowButton"

const PromoCard = () => {
 return (
   <Card className="mx-4 my-6 md:my-12 md:mx-auto md:max-w-6xl overflow-hidden border-white shadow-lg rounded-3xl">
     <CardContent className="p-6 md:p-12">
       <div className="grid grid-cols-1 md:grid-cols-2 md:gap-8">
         <div
           className="flex items-center justify-center max-h-72 mb-24 md:mb-0"
           style={{ aspectRatio: "4/3" }}
         >
           <BreathingOrb />
         </div>
         <div className="flex flex-col justify-center space-y-8 md:space-y-0">
           <div className="max-w-md mx-auto md:mx-0 space-y-6 md:space-y-4 text-center md:text-left">
             <p className="text-base text-muted-foreground">
               We are constantly bombarded with sensory information.
               Give your mind the peace it deserves.
             </p>
             <p className="text-base font-semibold text-foreground">
               Generative Science-Backed Guided Deep Rest is here.{" "}
             </p>
             <StartNowButton />
           </div>
         </div>
       </div>
     </CardContent>
   </Card>
 )
}

export default PromoCard